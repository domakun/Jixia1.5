...
function selectAllStrategy(first,max,cb) {
	var sql = 'select glb_id,title,glb_info,user_name,date,glb_img from glb inner join user on author_id=user_id order by date desc limit ?,?';
	var param = [first,max];
...
function searchGl(first,max,jd_addr,content,jd_name,date,author_id,cb) {
	var sql = 'select glb_id,title,glb_info,glb_img,user_name,date,jd_addr,jd_name,content from glb inner join user on author_id=user_id inner join jd on gladdr_id=jd_id ' +
...
function showSomestrategy(glb_id,cb) {
	var sql = 'select glb_id,title,date,user_name,glb_info,glb_img,content,head_img,sign,jd_name,jd_addr from glb inner join user on author_id=user_id inner join jd on gladdr_id=jd_id and glb_id = ?';
	dbutils.select(sql,[glb_id],function(result) {
		console.log(sql,glb_id);
